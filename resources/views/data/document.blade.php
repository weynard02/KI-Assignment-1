            ...
            <div class="card-body">
                <div class="d-flex flex-column">
                    <div class="form-group mb-3 p-2">
                        ...
                        <input type="hidden" class="form-control" id="realsymkey" value="{{$aesuser->document_key}}">
                    ...
                    <form action="{{ route('mail.document', ['key' => $user->id]) }}" method="post" enctype="multipart/form-data">
                        @csrf
                    ...
                    <div class="form-group mb-2 p-2 d-block visually-hidden" id="hiddendata">
                        <label class="fw-bold mb-2" style="font-size: 20px;">Here is {{$user->username}}'s document</label>
                        @php
                            $bkey = $aesuser->document_key;
                            $bkey = str_replace('/', '', $bkey);
                        @endphp
                        <a href="/download/aes/document/{{$aesuser->id}}/{{$bkey}}" class="btn btn-primary btn-sm">Download</a>
                    </div>
                </div>
            </div>
        </div>
